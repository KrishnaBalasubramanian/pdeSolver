...
def S_ph(X,t,sys):
    ...
    return sv
    #return np.ones(len(X))*1E2### used this for the testing purposes
    #return [0 for i in range(len(u))]
# uncomment to print the sources
...
V=np.zeros(nT)
for j in range(nT):
    V[j]=J*np.sum(getRho(J,soln[0,j,:]))
fig2,ax2 = plt.subplots()
ax2.plot(tGrid,V)
ax2.set_xlabel('time')
ax2.set_ylabel('Potential')
fig2.savefig('potentialPlot.png')
plt.close(fig2)
